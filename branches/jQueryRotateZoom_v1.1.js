...
Wilq32.PhotoEffect.prototype={
	...
	_Loader:(function()
	{
		...
		return function(parameters)
		{
			...
			var naturalWidth=this._img.naturalWidth;///my code
	...
	{	
		...
		if(this._parameters.dontZoom===false){ //all this has to be done only if zoom is enabled
			...
			// the number of pixels which will be added/subtracted in/from the image width everytime to zoom in/out
			this._incrementPxs = Math.round((this._naturalWidth)/(this._parameters.duration/14)); //my code
		}
		this._animate();
	},
    _animate:function()
             ...
             if (this._canvas||this._vimage||this._img) {
				 ...
				 if(dontZoom===false){//all this has to be done only if zoom is enabled
					if(Math.ceil(angle) + 1  < this._parameters.animateTo){		//check whether zooming in
						...
						if(jQuery(this._img).parent().css('position')=='absolute') // this is for the floating div in which the image is opened for zoom in
							jQuery(this._img).parent().css({ width: '+='+incrementPxs });
					}
					else if(Math.ceil(angle)  > this._parameters.animateTo){ 	//check whether zooming out
						jQuery(this._img).css({ width: '-='+incrementPxs }); 	// use the jquery css funcion to zoom out
						if(parseInt(jQuery(this._img).css('width'))==0) 		// if the image width becomes zero hide the parent div
							jQuery(this._img).parent().hide();
						else   													// else continue decreasing the parent's width 
							if(jQuery(this._img).parent().css('position')=='absolute') //considering the postion of the floating div will be absolute
								jQuery(this._img).parent().css({ width: '-='+incrementPxs });
					}
				}				
				// zoom related code ends here.....
             }
